...
def filter_pool(df: pd.DataFrame, gold_numbers: set[int]) -> pd.DataFrame:
    """Keep: component not-null, resolution_hours > 0, not in current gold."""
    mask = df["component"].notna() & (df["resolution_hours"] > 0)
    df = df[mask].copy()
    before = len(df)
    df = df[~df["number"].isin(gold_numbers)]
    logger.info(
        "After filter: %d issues (removed %d in current gold)", len(df), before - len(df)
    )
    return df
...
def main() -> None:
    ...
    for repo_key, repo_name in REPOS.items():
        ...
        pool = filter_pool(pool, gold_numbers_repo)
        pool["repo"] = repo_name
    ...
    audit = {
        ...
        "sampling_methodology": (
            ...
            "max 3 per component per bucket. Seed 42."
        ),
    }
