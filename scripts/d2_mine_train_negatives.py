"""D2: mine hard negatives for the fine-tune, drawn ONLY from training-pool issues.

Leakage guard: the candidate corpus for negative mining is restricted to issue numbers that
appear in the TRAINING pool (as query or original) for the given task -- eval-set issues are
never even candidates, let alone selected. This is the mining-time half of the leakage guard
(spec.md: "hard negatives drawn ONLY from training-pool issues"); scripts/d2_assert_leakage_guard.py
re-asserts the eval/train disjointness this depends on, called here as a pre-flight gate.

Uses the same off-the-shelf pretrained BAAI/bge-base-en-v1.5 embedder as D1's baseline (pure
inference to rank candidates, not the model being trained) -- consistent with every prior mining
pass in this project (w3_t2_mine_negatives.py).

Reads:  reports/d1_train_pool_{task}.json
        reports/d1_eval_set_{task}.json          (for the disjointness pre-flight only)
        data/processed/issues_{repo}.parquet
Writes: data/d2_hard_negatives_{task}.parquet
          columns: repo, query_number, original_number, neg_number, neg_rank, neg_score, neg_text
"""
...
MAX_BODY = 512
NEGATIVES_PER_PAIR = 5  # train pools are thin (264 / 1734 pairs) -- keep this modest
...
def mine_task(task: str, model: SentenceTransformer) -> pd.DataFrame:
    disjoint = assert_task_disjoint(task)  # hard-fails on any train/eval issue overlap
    ...
    # bidirectional positives per query issue, within THIS task's train pool only
...
def main() -> None:
    ...
    for task in TASKS:
        ...
        out = DATA / f"d2_hard_negatives_{task}.parquet"
