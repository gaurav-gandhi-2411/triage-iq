...
def split_repo(
    ...
    pairs_per_comp = repo_gold.groupby("component_id").size().reindex(range(len(comp_nodes)), fill_value=0)
    ...
    logger.info("[%s] Split: train=%d  val=%d  test=%d", repo,
...
def verify_no_leakage(result: pd.DataFrame) -> None:
    ...
    for repo in result["repo"].unique():
        ...
        for col in ("query_number", "original_number"):
            issue_splits = (
                sub.groupby(col.replace("_number", "_number"))["split"]
                .apply(set)
            )
...
def main() -> None:
    ...
    k8s_numbers = set(
        ...
        gold.loc[gold["repo"] == "kubernetes_kubernetes", "original_number"].astype(int)
    )
    vsc_numbers = set(
        gold.loc[gold["repo"] == "microsoft_vscode", "query_number"].astype(int)
    ) | set(
        gold.loc[gold["repo"] == "microsoft_vscode", "original_number"].astype(int)
    )
    ...
    is_cross_repo = (
        gold["query_number"].astype(int).isin(cross_repo_numbers)
        | gold["original_number"].astype(int).isin(cross_repo_numbers)
    )
    n_cross = is_cross_repo.sum()
    logger.info("Dropping %d cross-repo-contaminated pairs (%.1f%%)", n_cross, 100 * n_cross / len(gold))
    ...
    for repo in result["repo"].unique():
        ...
        logger.info(
            ...
            repo, len(sub), counts.get("train", 0), counts.get("val", 0),
            counts.get("test", 0), n_comps,
        )

    stats_path = "data/w3_split_stats.json"
