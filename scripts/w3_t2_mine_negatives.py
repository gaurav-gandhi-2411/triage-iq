...
INDEX_DIR = "data/models/dup_index_{repo}_bge_v2"  # v2: grown-corpus baseline index
...
def mine_repo(
    ...
    unique_q = repo_gold[["query_number", "query_title", "query_body"]].drop_duplicates(
        "query_number"
    )
    qnums = unique_q["query_number"].astype(int).tolist()
    qtexts = [_build_text(r["query_title"], r["query_body"]) for _, r in unique_q.iterrows()]
    ...
    records: list[dict] = []
    for qnum, scores, indices in zip(qnums, scores_all, indices_all, strict=True):
        ...
        originals = (
            repo_gold.loc[repo_gold["query_number"] == qnum, "original_number"].astype(int).tolist()
        )
...
def spot_check(
    ...
    for i, (_, row) in enumerate(sample.iterrows(), 1):
        ...
        def get_issue(num: int, issues=issues) -> tuple[str, str]:
...
def main() -> None:
    ...
    issues_map = {
        "kubernetes_kubernetes": pd.read_parquet(
    ...
    out = "data/w3_hard_negatives_v2.parquet"
    ...
    logger.info(
        ...
        per_pair.median(),
        per_pair.quantile(0.05),
        per_pair.quantile(0.95),
        per_pair.min(),
        per_pair.max(),
    )
    logger.info(
        "Score distribution: mean=%.4f  std=%.4f  min=%.4f  max=%.4f",
        result["neg_score"].mean(),
        result["neg_score"].std(),
        result["neg_score"].min(),
        result["neg_score"].max(),
    )
