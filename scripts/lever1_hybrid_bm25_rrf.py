...
via select_live_product_pairs() (277 k8s / 292 vscode pairs, only 72%/20% hand-verified genuine
per ADR-0032) against the stale served dup_index_* (k8s #1-15002 only, vscode 7,028 issues only),
with queries additionally truncated to query_body[:512] (never populated -- title-only in
practice). Now reads D1's canonical, hand-verified, disjoint eval sets (150/200 pairs, 100%
...
def eval_repo(repo: str, index_dir: str) -> dict:
    ...
    log.info("[%s] %d D1 canonical eval pairs", repo, len(pairs))

    dense_hits, bm25_hits, rrf_hits, weighted_hits = [], [], [], []
    ...
    for row in pairs.itertuples():
        ...
        bm25_hit = [n == target_num for n in bm25_ranked[:K_MAX]]
    ...
    bm25_r5 = np.array([float(any(h[:5])) for h in bm25_hits])
    rrf_r5 = np.array([float(any(h[:5])) for h in rrf_hits])
    weighted_r5 = np.array([float(any(h[:5])) for h in weighted_hits])

    bm25_lo, bm25_hi, bm25_delta = paired_bootstrap_ci(dense_r5, bm25_r5)
    ...
    result = {
        ...
        "bm25_only": recall_table(bm25_hits),
        "hybrid_rrf": recall_table(rrf_hits),
        "hybrid_weighted": recall_table(weighted_hits),
        "bm25_vs_dense_r5_delta_pp": round(bm25_delta * 100, 2),
        "bm25_vs_dense_r5_ci95_pp": [round(bm25_lo * 100, 2), round(bm25_hi * 100, 2)],
...
def main() -> None:
    results = [eval_repo(r["repo"], r["index_dir"]) for r in REPOS]

    for r in results:
        log.info(
            "[%s] BM25-only R@5=%.4f  dense R@5=%.4f  RRF R@5=%.4f  delta=%.2fpp  CI=%s  ships=%s",
            r["repo"],
            r["bm25_only"]["recall_at_5"],
