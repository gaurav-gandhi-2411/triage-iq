...
MODEL_DIRS = {
    "combined": "data/models/bge_finetuned_combined_v2",
    "k8s": "data/models/bge_finetuned_k8s_v2",
    "vsc": "data/models/bge_finetuned_vsc_v2",
}
...
def load_triplets(
    ...
    hn_idx = hard_negs.sort_values(
        ["repo", "query_number", "original_number", "neg_rank"]
...
def eval_val_r5(
    ...
    val_rows = split_df[
        (split_df["split"] == "val")
        & (split_df["repo"].isin(repos))
        & (split_df["stratum"] != "train_only")
    ]
    if val_rows.empty:
        return 0.0
...
def train_model(
    ...
    logger.info(
        "=== Training: %s | %d triplets | device=%s | out=%s ===",
        name,
        len(train_dataset),
        device,
        out_dir,
    ...
    for epoch in range(EPOCHS):
        ...
        for batch in loader:
            ...
            # Encode
            emb_a = F.normalize(
                mean_pool(model(a_ids, a_mask).last_hidden_state, a_mask), p=2, dim=1
            )
            emb_p = F.normalize(
                mean_pool(model(p_ids, p_mask).last_hidden_state, p_mask), p=2, dim=1
            )
            emb_n = F.normalize(
                mean_pool(model(n_ids, n_mask).last_hidden_state, n_mask), p=2, dim=1
            )
...
def main() -> None:
    ...
    hard_negs = pd.read_parquet("data/w3_hard_negatives_v2.parquet")
    ...
    r5_combined = train_model(
        "combined", ds_combined, split_df, gold, REPOS, MODEL_DIRS["combined"]
    )
    ...
    with open("reports/w3_t4_val_results_v2.json", "w") as f:
        json.dump(report, f, indent=2)
    logger.info("Report → reports/w3_t4_val_results_v2.json")
